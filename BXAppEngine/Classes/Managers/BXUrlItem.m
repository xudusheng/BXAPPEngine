...
-(BXUrlItem *)merge:(BXUrlItem *)urlItem{
    //urlItem的优先级高
    if (!(self.xdid.length && [self.xdid isEqualToString:urlItem.xdid])) {
        return nil;
    }
    
    BXUrlItem * newItem = [[BXUrlItem alloc] init];
...
-(NSString *)url{
    ...
    for (NSString *key in keyArr) {
        if (!urldic[key]) {
            ...
            BXUrlItem * urlItem =  [BXConfigItem globalParams];
    ...
    //handle timestamp
    int timestampRefreshInterval = [[[BXConfigManager sharedManager].configItem urlItemByXdid:@"xd.service.interval" paramPath:@"timestamp"] intValue];
