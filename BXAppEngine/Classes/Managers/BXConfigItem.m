...
static BXUrlItem * sGlobalParams;

@interface BXConfigItem ()

@property (nonatomic, strong)   NSMutableDictionary * testUrlDictionary;


@end
@implementation BXConfigItem
#pragma mark -
...
-(id)initConfigItemWithJsonStr:(NSString *)jsonStr DefaultContry:(NSString *)defaultCountry DefaultLanguage:(NSString *)defaultLanguage{
    self = [self init];
    if (self){
        [BXConfigItem writeConfigWithJsonStr:jsonStr];
...
-(id)initConfigItemBySystemLanguageWithJsonStr:(NSString *)jsonStr{
    return   [self initConfigItemWithJsonStr:jsonStr DefaultContry:nil DefaultLanguage:[BXLocalizableManager getAppDefaultLanguage]];
}
...
-(BOOL)parseJsonStr:(NSString *)jsonsrt{
    ...
    for (NSDictionary * dic in dataSource) {
        BXUrlItem * item = [[BXUrlItem alloc] init];
...
-(void)configProperties{
    ...
    for (NSString * xdidKey in [self.platforms allKeys]) {
        BXUrlItem * platform =  [self.platforms objectForKey:xdidKey];
        BXUrlItem * servicesItem =  [self.services objectForKey:xdidKey];
    ...
    for (NSString * countryCode in [self.country allKeys]) {
        ...
        if (servicesInCountry.count) {
            for (NSString * xdidKey in [servicesInCountry allKeys]) {
                BXUrlItem * countryItem =  [servicesInCountry objectForKey:xdidKey];
                BXUrlItem * servicesItem =  [self.services objectForKey:xdidKey];
    ...
    for (NSString * languageCode in [self.language allKeys]) {
        ...
        if (servicesInLanguage.count) {
            for (NSString * xdidKey in [servicesInLanguage allKeys]) {
                BXUrlItem * languageItem =  [servicesInLanguage objectForKey:xdidKey];
                BXUrlItem * servicesItem =  [self.services objectForKey:xdidKey];
            ...
            if ([titlekey isEqualToString:groupDic[testName]]) {
                for (NSDictionary *cellDic in groupDic[testItems]) {
                    if ([cellDic[testName] isEqualToString:usedConfigs[titlekey]]) {
                        for (BXUrlItem *urlItem in cellDic[testItems]) {
                            BXUrlItem * servicesItem =  [self.services objectForKey:urlItem.xdid];
                            if (servicesItem) {
...
-(BXUrlItem *)urlItemByXdid:(NSString *)xdid{
    BXUrlItem * urlItem;
    urlItem = (BXUrlItem *)[self.services objectForKey:xdid];
    if (urlItem) {
        [urlItem urlItemByLocalized];
        return  urlItem;
    }
    return [[self.appParams objectForKey:xdid] urlItemByLocalized];
}
...
-(id)urlItemByXdid:(NSString *)xdid  paramPath:(NSString *)paramPath{
    ...
    return [((BXUrlItem *)[[self.appParams objectForKey:xdid] urlItemByLocalized]) paramsValueBykey:paramPath];
}
...
-(void)readConfigFileAnConfigProperties:(NSString *)filePath{
    NSString * jsonStr = [BXConfigItem readConfig:filePath];
...
//    return seld.defaultLanguage;
    return  [BXLocalizableManager matchSimilarLanguageWithSelectedLanuage:self.defaultLanguage localLanguageArray:[self.language allKeys]];
}
