...
#define BXLocalizedString(key, comment) \
[[BXLocalizableManager sharedManager] localizableString:(key) placeholder:(comment)]

typedef void(^XDSLocalizableUpdateFinishedBlock)(NSError *error);

@interface BXLocalizableManager : BXBaseManager
